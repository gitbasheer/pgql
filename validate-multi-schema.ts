...
async function main() {
  ...
  console.log('\nLoading queries...');
  const queriesPath = await fs
    .access('./extracted-queries-with-fragments.json')
  ...
  for (const queryName of problemQueries) {
    ...
    if (result) {
      ...
      console.log(
        `  Detected Schema: ${result.classification.detectedSchema} (${Math.round(result.classification.confidence * 100)}%)`,
      );
  ...
  await fs.writeFile(
    './multi-schema-validation-report.json',
