    ...
    const errorPatterns = [
      {
        ...
        solution:
          'Make sure your GraphQL schema file exists at the specified path. Common locations:\n' +
          '- `data/schema.graphql`\n' +
          '- `src/schema.graphql`\n' +
      ...
      },
      {
        ...
        solution:
          "Some queries don't match your schema. Options:\n" +
      ...
      },
      {
        ...
        solution:
          'The pg-migration tool might not be built. Try:\n' +
      ...
      },
      {
        ...
        solution:
          'File system access issue. Check:\n' +
      ...
      },
      {
        ...
        solution:
          'Invalid GraphQL syntax detected. Check:\n' +
...
class ResponseFormatter {
  ...
  static formatApplyResult(rawOutput: string, hasBackup: boolean): string {
    const success =
      !rawOutput.toLowerCase().includes('error') && !rawOutput.toLowerCase().includes('failed');

    let formatted = success ? '✅ **Changes Applied Successfully**\n\n' : '❌ **Apply Failed**\n\n';
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              autoApply: {
                type: 'boolean',
                description: 'Apply changes automatically if safe',
                default: false,
              },
              confidenceThreshold: {
                type: 'number',
                description: 'Min confidence for auto-apply (0-100)',
                default: 90,
              },
            },
        ...
        switch (name) {
          ...
          case 'extract_queries': {
            ...
            if (existsSync(outputPath)) {
              ...
              queryCount = extractedData.queries
                ? extractedData.queries.length
          ...
          case 'assess_migration_impact': {
            ...
            try {
              ...
              // Risk assessment
              result += '\n3. Risk Assessment:\n';
              const queryCount = JSON.parse(
                readFileSync(join(this.projectRoot, queriesFile), 'utf8'),
              ).length;
              if (queryCount > 50) {
          ...
          case 'run_migration_pipeline': {
            ...
            try {
              ...
              execSync(`pnpm run extract ${directory} -o ${output}-extracted.json`, {
                cwd: this.projectRoot,
              });
              ...
              if (autoApply) {
                ...
                execSync(`pnpm run apply -i ${output}-transformed.json --backup`, {
                  cwd: this.projectRoot,
                });
