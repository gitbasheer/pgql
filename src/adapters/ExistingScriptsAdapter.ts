...
export class ExistingScriptsAdapter {
  ...
  async transformOperation(operation: GraphQLOperation): Promise<CodeChange[]> {
    ...
    try {
      ...
      const transformScript = path.join(this.scriptsBasePath, 'transformation/enhanced-graphql-codemod-v2.js');
  ...
  async validateOperations(source: string, schemaPath: string): Promise<{
    valid: boolean;
    errors: Array<{
      operation: string;
      message: string;
  ...
  private detectPattern(oldQuery: string, newQuery: string): string {
    ...
    if (newQuery.includes('ventures') && oldQuery.includes('allVentures')) return 'root-query-migration';
