      ...
      await service.captureBaseline([
        {
          ...
          type: 'query',
          filePath: 'test.js',
          location: { line: 1, column: 1, file: '/Users/balkhalil/gd/demo/pg-migration-620/src/test/validator/ResponseCaptureService.cookie.test.js' },
          hash: 'test-hash',
        },
      ]);
    ...
    it('should log info for SSO authentication type', async () => {
      ...
      expect(mockedAxios.create).toHaveBeenCalledWith(
        expect.objectContaining({
          headers: expect.objectContaining({
            Cookie:
              'auth_idp=mock-auth-idp; cust_idp=mock-cust-idp; info_cust_idp=mock-info-cust-idp; info_idp=mock-info-idp',
          }),
          withCredentials: true,
        }),
      );
