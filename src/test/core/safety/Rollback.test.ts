...
describe('RollbackSystem', () => {
  ...
  describe('createRollbackPlan', () => {
    it('should create an immediate rollback plan by default', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(`Created rollback plan: ${plan.id} with strategy: immediate`);
    });
    ...
    it('should create a gradual rollback plan when specified', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(`Created rollback plan: ${plan.id} with strategy: gradual`);
    });
  });
  ...
  describe('executeRollback', () => {
    describe('immediate rollback', () => {
      it('should immediately disable all operations', async () => {
        ...
        expect(logger.info).toHaveBeenCalledWith(`Rollback completed successfully for plan: ${plan.id}`);
      });

      it('should throw error for non-existent plan', async () => {
        await expect(
          rollbackSystem.executeRollback('non-existent-plan')
        ).rejects.toThrow('Rollback plan not found: non-existent-plan');
      });
