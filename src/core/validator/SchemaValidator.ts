...
export class SchemaValidator {
  ...
  private createActionableError(
    ...
    if (error.message.includes('Cannot query field')) {
      ...
      baseError.suggestion =
        'Check that all variables are properly defined and match their expected types.';
    }
  ...
  async validateQuery(
    ...
    if (validationErrors.length > 0) {
      errors.push(...validationErrors.map((err) => this.createActionableError(err, queryString)));
    }
        ...
        Field: (node) => {
          ...
          if (fieldDef && parentType) {
            ...
            if (
              ...
              !warnings.some((w) => w.field === `${typeName}.${fieldName}`)
            ) {
              warnings.push({
  ...
  generateValidationReport(results: Map<string, ValidationResult>): {
    total: number;
    valid: number;
    invalid: number;
    warnings: number;
  ...
  private preprocessTemplateLiterals(query: string): string {
    ...
    processedQuery = processedQuery.replace(/"\$\{[^}]+\}"/g, '"placeholder"');

    // Handle numeric interpolations: ${expr} → 0 (when in numeric context)
    processedQuery = processedQuery.replace(/:\s*\$\{[^}]+\}/g, ': 0');
    ...
    processedQuery = processedQuery.replace(/(\w+V)\$\{[^}]+\}/g, '$1_Dynamic');
