...
export class ASTCodeApplicator {
  ...
  private applyToTaggedTemplate(
    ...
    if (transformation.preserveInterpolations && quasi.expressions.length > 0) {
      ...
      const newTemplateElement = t.templateElement(
        ...
        true,
      ); // tail = true since it's the only element
  ...
  private findMatchingTransformation(
    ...
    const contentMatch = transformations.find((t) => {
      ...
      const positionDiff =
        Math.abs((astNode.start ?? 0) - node.start) + Math.abs((astNode.end ?? 0) - node.end);
      if (positionDiff > 100) {
      ...
      return (
        transformationOriginal === nodeContentNormalized ||
        // Also check if it's a substring match (for cases with interpolations)
        (nodeContentNormalized.includes(transformationOriginal) &&
          transformationOriginal.length > 10) ||
        (transformationOriginal.includes(nodeContentNormalized) &&
          nodeContentNormalized.length > 10)
      );
    });
  ...
  private async validateChanges(
    ...
    try {
      ...
      throw new Error(
        `Generated invalid JavaScript/TypeScript: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
