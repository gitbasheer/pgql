...
export class AdvancedVariantExtractor {
  ...
  private async extractFromFile(filePath: string): Promise<{ queries: ExtractedQuery[]; variants: AdvancedQueryVariant[] }> {
    const content = await fs.readFile(filePath, 'utf-8');
    ...
    try {
      ...
      traverse(ast, {
        ...
        TaggedTemplateExpression: (nodePath: any) => {
          logger.debug(`Found TaggedTemplateExpression, tag: ${nodePath.node.tag.type} ${nodePath.node.tag.name}`);
      ...
      expressions.forEach((expr: any, index: number) => {
        ...
        if (expr.type === 'ConditionalExpression') {
          ...
          if (prevText.trim().endsWith('...') && this.isFragmentExpression(expr)) {
            ...
            logger.debug(`Condition: ${conditionVar}, True: ${trueFragment}, False: ${falseFragment}`);
  ...
  private isFragmentExpression(expr: any): boolean {
    logger.debug(`Checking fragment expression: consequent=${expr.consequent?.type}, alternate=${expr.alternate?.type}`);
      ...
      for (let i = 0; i < quasis.length; i++) {
        ...
        if (i < expressions.length) {
          ...
          if (dynFrag) {
            ...
            const fragmentName = combo[dynFrag.conditionVar] ? dynFrag.trueFragment : dynFrag.falseFragment;
