...
export class VariantAwareExtractor extends GraphQLExtractor {
  ...
  private extractDynamicPatterns(
    context: string,
    queryContent: string,
  ): Array<{
    type: 'fragment' | 'field';
  ...
  private generateVariants(query: ExtractedQuery, patterns: Array<any>): ExtractedQuery[] {
  ...
  private createVariant(
    ...
    const variantContent = query.content;
    ...
    const config = {
      ...
      summary: {
        ...
        totalQueriesWithVariants: new Set(
          Array.from(this.variantConfig.variables.values()).flatMap((v) => v.usedIn),
        ).size,
      },
    };
  ...
  async generateVariantReport(): Promise<{
    ...
    const queriesWithVariants = new Set(variables.flatMap((v) => v.usedIn)).size;
