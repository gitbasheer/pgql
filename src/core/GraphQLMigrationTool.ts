...
export class GraphQLMigrationTool {
  ...
  private async applyTransformation(filePath: string, query: any, result: any): Promise<void> {
    ...
    if (query.sourceAST) {
      ...
      if (reExtractedQuery) {
        ...
        logger.error(
          `GraphQLExtractor needs to be updated to preserve sourceAST. Query transformation cannot proceed safely without source mapping.`,
        );
        throw new Error(
          `Unable to preserve source AST for query in ${filePath}. The extractor needs to implement sourceAST preservation.`,
        );
      } else {
        throw new Error(
          `Unable to find matching query in ${filePath}. AST-based transformation is required for safety.`,
        );
      }
    }
  }
