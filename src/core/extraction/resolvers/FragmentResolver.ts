...
export class FragmentResolver {
  ...
  private async extractFragmentsFromContent(
    content: string,
    filePath: string,
  ): Promise<FragmentDefinition[]> {
  ...
  private async resolveQuery(query: ExtractedQuery): Promise<ResolvedQuery> {
    ...
    if (this.context.options.inlineFragments) {
      resolvedQuery.resolvedContent = this.buildResolvedContent(
        query,
        resolvedQuery.resolvedFragments,
      );
    }
