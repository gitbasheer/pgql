...
export class FragmentResolver {
  ...
  private async resolveQuery(
    ...
    if (usedFragments.length > 0) {
      ...
      const fragmentDefinitions = usedFragments.map((f) => print(f)).join('\n\n');
  ...
  private async extractFragmentsFromJavaScript(
    content: string,
    filePath: string,
  ): Promise<FragmentDefinitionNode[]> {
      ...
      if (fragmentContent.includes('fragment ')) {
        ...
        if (validation.isValid && validation.ast) {
          ...
          logParsingError(
            fragmentContent,
            validation.error!,
            `exported fragment ${fragmentName} in ${filePath}`,
          );
        }
      }
    }
  ...
  async findAndLoadFragmentFiles(
    directory: string,
  ): Promise<Map<string, FragmentDefinitionNode[]>> {
