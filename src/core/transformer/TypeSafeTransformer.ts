...
export class TypeSafeTransformer {
  ...
  transform(code: string, context: TransformContext): Result<TransformResult, TransformError> {
    // Parse phase
    ...
    try {
      const transformed = queries.map((query) => this.applyTransformations(query, this.rules));
  ...
  private applyTransformations(query: DocumentNode, rules: MigrationRule[]): DocumentNode {
    return visit(query, {
      Field(node) {
        const matchingRule = rules.find((rule) => node.name.value === rule.from.field);
