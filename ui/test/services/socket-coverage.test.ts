...
describe('SocketService Additional Coverage', () => {
  ...
  it('should handle pipeline events with authentication', () => {
    ...
    const stageHandler = mockSocket.on.mock.calls.find(
      (call) => call[0] === 'pipeline:stage'
    )?.[1];
    stageHandler?.({
      stage: 'testing',
      status: 'in_progress',
  ...
  it('should handle real API testing events', () => {
    ...
    const realApiHandler = mockSocket.on.mock.calls.find(
      (call) => call[0] === 'realapi:test:started'
    )?.[1];
    realApiHandler?.({
      queryName: 'GetUser',
  ...
  it('should handle pipeline completion events', () => {
    ...
    const completedHandler = mockSocket.on.mock.calls.find(
      (call) => call[0] === 'pipeline:completed'
    )?.[1];
    completedHandler?.({
      pipelineId: 'test-123',
      status: 'success',
  ...
  it('should handle real API baseline save events', () => {
    ...
    const baselineHandler = mockSocket.on.mock.calls.find(
      (call) => call[0] === 'realapi:baseline:saved'
    )?.[1];
    baselineHandler?.({
      queryName: 'GetUser',
  ...
  it('should handle real API test completed events', () => {
    ...
    const completedHandler = mockSocket.on.mock.calls.find(
      (call) => call[0] === 'realapi:test:completed'
    )?.[1];
    completedHandler?.({
      queryName: 'GetUser',
      success: true,
