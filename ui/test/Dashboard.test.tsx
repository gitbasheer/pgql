...
describe('Dashboard', () => {
  ...
  it('renders the dashboard header', () => {
    ...
    expect(
      screen.getByText(
        'Real-time monitoring for your GraphQL migration pipeline'
      )
    ).toBeInTheDocument();
  });
  ...
  it('renders pipeline configuration form', () => {
    ...
    expect(
      screen.getByLabelText('Test API URL (Optional)')
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText('Test Account ID (Optional)')
    ).toBeInTheDocument();
  });
  ...
  it('displays pipeline progress section', () => {
    ...
    expect(
      screen.getByText('Pipeline will appear here once started...')
    ).toBeInTheDocument();
  });
  ...
  it('should disable vnext button during testing', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /Testing vnext/i })
    ).toBeDisabled();
  });
  ...
  it('should handle vnext test errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith(
        'vnext testing failed: Network error during extraction'
      );
    });
  });
});
