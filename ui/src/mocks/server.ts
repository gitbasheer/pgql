...
app.post('/api/extract', (req: any, res: any) => {
  ...
  if (!repoPath || !schemaEndpoint) {
    return res.status(400).json({
      message: 'Missing required fields: repoPath and schemaEndpoint',
    });
  }
  ...
  const queries = [
    {
      ...
      transformation:
        pipeline.stages.transformation === 'completed'
          ? {
              transformedQuery:
  ...
  return {
    ...
    },
    {
      query: {
        queryName: 'listPosts',
        content:
          'query listPosts { posts { id title content author { name } } }',
  ...
  const stages = [
    ...
    },
    {
      ...
      logs: [
        ...
        {
          level: 'info',
          message: 'Query getUser: simple query with variables',
        },
        {
          level: 'info',
          message: 'Query listPosts: nested query with fragments',
        },
      ],
    },
    {
      ...
      logs: [
        ...
        {
          level: 'warn',
          message: 'Query listPosts uses deprecated field "content"',
        },
