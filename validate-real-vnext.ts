...
async function getAuthConfig(): Promise<VnextAuthConfig | null> {
  ...
  const hasAllEnvCookies = Object.values(envCookies).every(cookie => 
    cookie && cookie.length > 10 && !cookie.includes('test-')
  );
...
async function extractVnextQueries(vnextPath: string) {
  ...
  try {
    ...
    logger.error('❌ Query extraction failed:', error instanceof Error ? error.message : 'Unknown error');
    throw error;
  }
}
...
async function validateRealVnext() {
  ...
  try {
    ...
    console.log(`✅ Authentication: ${auth.source} (expires: ${auth.expiresAt?.toISOString() || 'N/A'})`);
    ...
    console.error('❌ Real vnext validation failed:', error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  }
}
